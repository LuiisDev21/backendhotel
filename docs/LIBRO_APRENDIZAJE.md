# 📚 Libro de Aprendizaje: Sistema de Reservas de Hotel con FastAPI
...
#### HabitacionResponse
- Hereda de `HabitacionBase` + campos adicionales
...
**Composición de Dependencias**: Reutiliza `ObtenerUsuarioActual` y agrega validación adicional.
...
from app.core.dependencies import ObtenerAdministradorActual
...
@router.post("", 
    ...
    dependencies=[Depends(ObtenerAdministradorActual)]
)
def CrearHabitacion(
...
Agregar validación en `HabitacionService.CrearHabitacion`:
...
Este libro cubre todos los aspectos del proyecto:

1. **Arquitectura**: Capas bien definidas
2. **Base de Datos**: Modelos y relaciones
3. **API REST**: Endpoints bien estructurados
4. **Seguridad**: Autenticación JWT robusta
5. **Buenas Prácticas**: Código mantenible y escalable
